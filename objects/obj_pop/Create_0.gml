...
// The actual function 'initialize_from_profile' defined later in this event file will simply not be executed
// as part of this instance's creation if all logic is handled before its definition.

// The old initialize_from_profile function is below this line.
// All its functionality should now be covered by the code above, using self.entity_data.
// It can be safely removed from this file once confirmed.
// For now, it remains, but should not be executed by new obj_pop instances.

/*
/// obj_pop - Deprecated Initialize From Profile Method
///
/// Purpose:
///    This method was used for initializing pop instances from profile data.
///    It is now deprecated and replaced by direct initialization in the Create event.
///
/// Metadata:
///    Summary:         Deprecated method for backward compatibility.
///    Usage:           Should not be used in new code. Existing saves may call this.
///    Version:        1.10 - May 22, 2025 (Deprecated)
///    Dependencies:  EntityState (enum), scr_generate_pop_details, spr_man_idle.
...
initialize_from_profile = function() {
    ...
    self.image_speed = (is_undefined(self.sprite_index) || self.sprite_index == noone) ? 0 : 1; 
    image_scale = self.image_bas // This line was incomplete in the original file
    ...
    if (struct_exists(_profile, "innate_trait_profile_paths") && ds_exists(_profile.innate_trait_profile_paths, ds_type_list)) {
        ...
        for (var i = 0; i < ds_list_size(_trait_paths_list); i++) {
            ...
            if (!is_undefined(_trait_profile_data)) {
                ...
                show_debug_message("WARNING (obj_pop Create for " + self.pop_identifier_string + "): Innate trait profile ID \'" + string(_trait_id_string) + "\' not found in GameData. Trait not added.");
            }
        }
    } else {
    ...
    // Example: show_debug_message("DEBUG (obj_pop Create): Assigned sprites - Idle: " + string(self.spr_idle) + ", Walk Prefix: " + self.spr_walk_prefix_string + ", Portrait: " + string(self.spr_portrait) + ", Death: " + string(self.spr_death) + ".");
};
#endregion
*/
// The line `image_scale = self.image_bas` was incomplete in the original initialize_from_profile.
// It has been replaced by `self.image_xscale` and `self.image_yscale` initialization from `_data.base_scale` above.
// The entire initialize_from_profile function can now be deleted.
// Deleting the old initialize_from_profile method:
// (The following lines will be deleted by the tool)
// =========================================================================
// X. INITIALIZE FROM PROFILE METHOD (Called by scr_spawn_system)
// =========================================================================
// #region X.1 Initialize From Profile Method
// initialize_from_profile = function() {
// ... (all the old function code) ...
